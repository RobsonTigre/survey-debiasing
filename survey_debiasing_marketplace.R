# ==============================================================================
# Survey debiasing simulation: marketplace one-hour delivery
# ==============================================================================
#
# Companion script for the Substack post at posts/2026-04-survey-debiasing.md
# ("How to actually use survey and feedback responses from your users").
#
# This script simulates a marketplace survey where 150,000 users were invited
# to answer whether they'd use a new one-hour delivery tier, but only 1,001
# responded.
#
# The problem: Respondents differ systematically from the full population
# (they're more engaged, older, more male, spend more on the platform).
#
...
population <- tibble(
  ...
  # Annual refunds: mean $1,747, correlated with spend
  # Users who spend more also tend to have higher refund volumes
...
cat("- Has revenue:", percent(mean(population$has_made_purchase), accuracy = 0.1), "\n")
cat("- Mean annual spend:", dollar(mean(population$annual_spend)), "\n")
cat("- Mean annual refunds:", dollar(mean(population$annual_refunds)), "\n")
...
population <- population %>%
  mutate(
    ...
    logit_response = -6.5 +                      
                     ...
                     0.6 * spend_std +        # High depositors respond more
                     0.5 * refunds_std +     # High withdrawers respond more
...
cat("- Has revenue:", percent(mean(respondents$has_made_purchase), accuracy = 0.1), "\n")
cat("- Mean annual spend:", dollar(mean(respondents$annual_spend)), "\n")
cat("- Mean annual refunds:", dollar(mean(respondents$annual_refunds)), "\n")
...
# ------------------------------------------------------------------------------
# STEP 3: Assign survey responses (interest variable = user interested in one-hour delivery)
# ------------------------------------------------------------------------------

# Among respondents, 40.9% express interest ("Very likely" or "Somewhat likely")
# But interest correlates with engagement!
# Users who are highly engaged are ALSO more interested in one-hour delivery.
...
respondents <- respondents %>%
  mutate(
    ...
    logit_interest = -0.8 +                     # Base interest rate
                     ...
                     0.3 * spend_std,        # High value users MORE interested
...
cat("- Respondents with Interest = 1:", sum(respondents$interest), # 409 users interested in one-hour delivery
    paste0("(", percent(mean(respondents$interest), accuracy = 0.1), ")\n"))
cat("- This represents those who answered 'Very likely' or 'Somewhat likely'\n")
...
cat("   ", percent(naive_estimate, accuracy = 0.1), "would use one-hour delivery\n")
...
# 2.1 raw weights:
cat("   ", percent(debiased_raw, accuracy = 0.1), "would use one-hour delivery\n")
...
# 2.2. trimmed weights
cat("   ", percent(debiased_trimmed, accuracy = 0.1), "would use one-hour delivery\n")
...
cat("   ", percent(debiased_stabilized, accuracy = 0.1), "would use one-hour delivery\n")
...
# Assumption: Once you control for observed characteristics (age, spend, app opens, etc.), there are no other hidden factors that affect both who responds and their interest in one-hour delivery.
...
# Assumption: Variables like spend, age, app opens are measured accurately.